...
interface IStrategyV5 is IStrategyV5Agent, IAsProxy {
    ...
    function liquidate(uint256[8] calldata _amounts, uint256 _minLiquidity, bool _panic, bytes[] calldata _params)
        external
        returns (uint256 totalRecovered);
    function invest(uint256[8] calldata _amounts, bytes[] calldata _params) external returns (uint256 totalInvested);
    ...
    function compound(uint256[8] calldata _amounts, bytes[] calldata _harvestParams, bytes[] calldata _investParams)
        external
        returns (uint256 totalHarvested, uint256 totalInvested);
    function updateAsset(address _asset, bytes memory _swapData, uint256 _exchangeRateBp) external;
