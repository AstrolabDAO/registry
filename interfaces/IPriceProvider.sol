...
interface IPriceProvider {
    ...
    function convert(address _base, uint256 _amount, address _quote) external view returns (uint256);
    function setFeed(address _asset, bytes32 _feed, uint256 _validity) external;
    function setFeeds(address[] memory _assets, bytes32[] memory _feeds, uint256[] memory _validities) external;
